...
describe('compose-go parsing & validation', () => {
	describe('basic parsing', () => {
		...
		it('should parse a simple compose file', async () => {
			const composition = await parse('test/fixtures/simple.yml');
	...
	describe('top-level field removal', () => {
		let warnStub: SinonStub;
		...
		it('should remove project name and version from composition', async () => {
			const composition = await parse('test/fixtures/compose/name.yml');
		...
		it('should reject top-level secrets', async () => {
			try {
				await parse('test/fixtures/compose/secrets.yml');
		...
		it('should reject top-level configs', async () => {
			try {
				await parse('test/fixtures/compose/configs.yml');
	...
	describe('services', () => {
		...
		it('should normalize a service config with all supported fields, including fields with multiple allowed syntaxes', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/full.yml',
			);
		...
		it('should reject forbidden service config fields', async () => {
			for (const field of SERVICE_CONFIG_DENY_LIST) {
				try {
					await parse(
						`test/fixtures/compose/services/unsupported/${field}.yml`,
					);
		...
		it('should error on long syntax depends_on config', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/depends_on_long_syntax.yml',
				);
				expect.fail(
					'Expected compose parser to error on long syntax depends_on config',
				);
			} catch (error) {
				expect(error).to.be.instanceOf(ServiceError);
				expect(error.message).to.equal(
		...
		it('should reject devices config with CDI syntax', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/devices_cdi_syntax.yml',
				);
				expect.fail(
		...
		it('should inject a service dependency if network_mode:service:${serviceName} is defined', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/network_mode.yml',
			);
		...
		it('should reject network_mode:container:${containerId}', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/network_mode_container.yml',
				);
				expect.fail(
		...
		it('should normalize networks config', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/networks.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should warn of oom_score_adj values under -900', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/oom_score_adj.yml',
			);
			expect(warnStub.callCount).to.equal(1);
			expect(warnStub.firstCall.args[0]).to.equal(
		...
		it('should inject a service dependency if pid:service:${serviceName} is defined', async () => {
			const composition = await parse('test/fixtures/compose/services/pid.yml');
		...
		it('should reject pid:container:${containerId}', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/pid_container.yml',
				);
				expect.fail(
		...
		it('should normalize port definitions to short syntax', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/ports.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should warn if ignored fields in service.ports are present and remove them', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/ports_ignored.yml',
			);
			expect(warnStub.callCount).to.equal(3);
			expect(warnStub.firstCall.args[0]).to.equal(
				'service.ports.name is not supported. Removing from the composition',
			);
			expect(warnStub.secondCall.args[0]).to.equal(
				'service.ports.app_protocol is not supported. Removing from the composition',
			);
			expect(warnStub.thirdCall.args[0]).to.equal(
				'service.ports.mode is not supported. Removing from the composition',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should reject invalid ports', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/ports_invalid.yml',
				);
		...
		it('should parse restart policy', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/restart.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should reject volumes of type bind', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_bind.yml',
				);
		...
		it('should reject volumes of type bind with relative paths', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_bind_relative.yml',
				);
				expect.fail(
		...
		it('should reject volumes of type image', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_image.yml',
				);
		...
		it('should reject volumes of type npipe', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_npipe.yml',
				);
		...
		it('should reject volumes of type cluster', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_cluster.yml',
				);
				expect.fail(
		...
		it('should reject invalid volume definitions', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_invalid.yml',
				);
				expect.fail(
		...
		it('should reject a composition where a volume definition is missing in top-level volumes', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volume_missing.yml',
				);
				expect.fail(
		...
		it('should convert allowed bind mounts to labels', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/volumes_allowed_bind_mount.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should not convert allowed bind mounts to labels if not all bind mounts associated with label are present', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/volumes_allowed_bind_mount_partial.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should normalize service volumes to short syntax', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/volumes.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should reject if service volume of type volume defines volume options', async () => {
			try {
				await parse(
					'test/fixtures/compose/build/unsupported/volume_options.yml',
				);
				expect.fail(
		...
		it('should reject if service volume of type tmpfs defines tmpfs options', async () => {
			try {
				await parse(
					'test/fixtures/compose/build/unsupported/tmpfs_options.yml',
				);
				expect.fail(
		...
		it('should allow short syntax tmpfs options', async () => {
			const composition = await parse(
				'test/fixtures/compose/build/unsupported/tmpfs_short_syntax_options.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should inject a service dependency if volumes_from is defined', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/volumes_from.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					one: {
		...
		it('should reject volumes_from which references container:${containerId}', async () => {
			try {
				await parse(
					'test/fixtures/compose/services/unsupported/volumes_from_container.yml',
				);
				expect.fail(
		...
		it('should add image as build tag if present', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/image_build.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should warn if service.container_name is present and remove it', async () => {
			const composition = await parse(
				'test/fixtures/compose/services/container_name.yml',
			);
			expect(warnStub.callCount).to.equal(1);
			expect(warnStub.firstCall.args[0]).to.equal(
	...
	describe('service.build', () => {
		...
		it('should normalize service build config', async () => {
			const composition = await parse(
				'test/fixtures/compose/build/supported_fields.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should normalize service build context paths', async () => {
			const composition = await parse(
				'test/fixtures/compose/build/context.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should reject remote build context', async () => {
			try {
				await parse('test/fixtures/compose/build/context_remote.yml');
		...
		it('should reject forbidden build config fields', async () => {
			for (const field of BUILD_CONFIG_DENY_LIST) {
				try {
					await parse(`test/fixtures/compose/build/unsupported/${field}.yml`);
	...
	describe('networks', () => {
		...
		it('should normalize network config', async () => {
			const composition = await parse(
				'test/fixtures/compose/networks/supported_fields.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should reject unsupported network config fields', async () => {
			for (const field of NETWORK_CONFIG_DENY_LIST) {
				try {
					await parse(
						`test/fixtures/compose/networks/unsupported/${field}.yml`,
					);
		...
		it('should reject non-bridge or non-default network drivers', async () => {
			try {
				await parse(
					'test/fixtures/compose/networks/unsupported/driver_custom.yml',
				);
				expect.fail(
		...
		it('should remove network.name', async () => {
			const composition = await parse(
				'test/fixtures/compose/networks/unsupported/name.yml',
			);
	...
	describe('volumes', () => {
		...
		it('should normalize volume config', async () => {
			const composition = await parse(
				'test/fixtures/compose/volumes/supported_fields.yml',
			);
			expect(composition).to.deep.equal({
				services: {
					main: {
		...
		it('should normalize a volume without configuration to an empty object', async () => {
			const composition = await parse('test/fixtures/compose/volumes/null.yml');
			expect(composition).to.deep.equal({
				services: {
					main: {
						image: 'alpine:latest',
		...
		it('should reject unsupported volume config fields', async () => {
			for (const field of VOLUME_CONFIG_DENY_LIST) {
				try {
					await parse(`test/fixtures/compose/volumes/unsupported/${field}.yml`);
		...
		it('should reject non-local or non-default volume drivers', async () => {
			try {
				await parse(
					'test/fixtures/compose/volumes/unsupported/driver_custom.yml',
				);
				expect.fail(
		...
		it('should remove volume.name', async () => {
			const composition = await parse(
				'test/fixtures/compose/volumes/unsupported/name.yml',
			);
	...
	describe('multiple file merging', () => {
		...
		it('should merge scalars by adding missing entries and merging conflicting ones', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/scalar-base.yml',
				'test/fixtures/compose/merge/scalar-override.yml',
			]);
		...
		it('should replace shell commands', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/shell-base.yml',
				'test/fixtures/compose/merge/shell-override.yml',
			]);
		...
		it('should merge mappings by adding missing entries and merging conflicting ones', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/mapping-base.yml',
				'test/fixtures/compose/merge/mapping-override-1.yml',
				'test/fixtures/compose/merge/mapping-override-2.yml',
			]);
		...
		it('should merge sequences by appending values', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/sequence-base.yml',
				'test/fixtures/compose/merge/sequence-override.yml',
			]);
		...
		it('should only append entries that do not violate uniqueness constraints', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/uniqueness-base.yml',
				'test/fixtures/compose/merge/uniqueness-override.yml',
			]);
		...
		it('should respect !reset YAML tag to override a previously set value', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/reset-base.yml',
				'test/fixtures/compose/merge/reset-override.yml',
			]);
		...
		it('should respect !override YAML tag to replace a previously set value', async () => {
			const composition = await parse([
				'test/fixtures/compose/merge/replace-base.yml',
				'test/fixtures/compose/merge/replace-override.yml',
			]);
	...
	describe('advanced features', () => {
		...
		it('should support fragments', async () => {
			const composition = await parse('test/fixtures/compose/fragments.yml');
		...
		it('should support extensions', async () => {
			const composition = await parse('test/fixtures/compose/extensions.yml');
	...
	describe('contract validation', () => {
		it('should validate contract labels', async () => {
			const composition = await parse('test/fixtures/contracts/valid.yml');
			expect(composition).to.deep.equal({
				services: {
					main: {
						image: 'alpine:latest',
		...
		it('should reject if label `io.balena.features.requires.sw.supervisor` uses a wrong version range', async () => {
			try {
				await parse('test/fixtures/contracts/invalid_sw_supervisor.yml');
				expect.fail(
		...
		it('should reject if label `io.balena.features.requires.sw.l4t` uses a wrong version range', async () => {
			try {
				await parse('test/fixtures/contracts/invalid_sw_l4t.yml');
				expect.fail(
		...
		it('should reject if label `io.balena.features.requires.arch.sw` uses an invalid architecture', async () => {
			try {
				await parse('test/fixtures/contracts/invalid_arch_sw.yml');
				expect.fail(
